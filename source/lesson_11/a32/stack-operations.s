...
_start:
	 ...
	 * be allocated:*/
	sub sp, #0x8 /* Allocates 8 bytes of memory from the stack by subtracting 8 
				  * from the value stored in the stack pointer.*/
	...
	str r4, [sp] /* Stores the value from r4 in the location referenced by the 
				  * stack pointer. This is argument 5.*/

	mov r4, #0x8 // Moves the value 8 into r4.
	str r4, [sp, #0x4] /* Stores the value from r4 in location of referenced by 
					    * the stack pointer offset by 4 bytes.*/
	 ...
	 * caller-saved: The calling procedure is
	 * responsible for preserving their value. Conversely, registers r4 - r11 
	 * are callee-saved: If the called procedure modifies them, it is 
	 * responsible for preserving their original values.
	 *
	...
	bl add_nums /* Branches to the add_nums procedure and sets the link register
				 * to the location of the next instruction from the current 
				 * procedure.*/

	add sp, #0x8 /* Deallocates the additional 8 bytes of memory that were 
				  * allocated from the stack earlier now that they're no longer 
				  * needed.*/
...
add_nums:
	...
	ldr r3, [sp] /* Load the value stored at the location referenced by the 
				  * stack pointer into r3.*/
	add r0, r3 // Add the values from r0 and r3 and store the result in r0.

	ldr r3, [sp, #0x4] /* Load the value stored at the location referenced by 
						* the stack pointer offset by 4 bytes into r3.*/
	add r0, r3 // Add the values from r0 and r3 and store the result in r0.
