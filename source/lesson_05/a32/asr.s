...
_start:
	/* The Arithemetic Shifth Right (ASR) operator shifts the value stored in a register to the right by the specified number of
	 * bits and stores the result in a specified destination register. Unlike the LSR operator, however, the ASR operator pads the
	 * register with the value of the sign bit. The number of bits to shift by can be provided via either register or an immediate
	 * value. The source and destination registers can be the same.
         *
